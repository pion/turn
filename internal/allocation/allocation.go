...
func (a *Allocation) AddPermission(p *Permission) {
	...
	if a.callback != nil {
		if u, ok := p.Addr.(*net.UDPAddr); ok {
			a.log.Trace("Calling OnPermissionCreated event handler")
...
func (a *Allocation) RemovePermission(addr net.Addr) {
	...
	if a.callback != nil {
		if u, ok := addr.(*net.UDPAddr); ok {
			a.log.Trace("Calling OnPermissionDeleted event handler")
			a.callback(EventHandlerArgs{
...
func (a *Allocation) AddChannelBind(c *ChannelBind, lifetime time.Duration) error {
	...
	if channelByNumber == nil {
		...
		if a.callback != nil {
			a.log.Trace("Calling OnChannelCreated event handler")
			a.callback(EventHandlerArgs{
...
func (a *Allocation) RemoveChannelBind(number proto.ChannelNumber) bool {
	...
	for i := len(a.channelBindings) - 1; i >= 0; i-- {
		if a.channelBindings[i].Number == number {
			if a.callback != nil {
				a.log.Trace("Calling OnChannelDeleted event handler")
				a.callback(EventHandlerArgs{
