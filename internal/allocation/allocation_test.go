...
func subTestPacketHandler(t *testing.T) {
	...
	fiveTuple := NewFiveTuple(clientListener.LocalAddr(), turnSocket.LocalAddr(), UDP)
	a, err := m.CreateAllocation(fiveTuple, clientListener.LocalAddr(), turnSocket, 0, proto.DefaultLifetime)
